...
<%@ taglib prefix="xlat" uri="futuretense_cs/xlat.tld" %>
<%//
// OpenMarket/Xcelerate/AssetType/SiteNavigation/SiteplanMgt/deletePrompt
//
// INPUT
//
// OUTPUT
...
<satellite:form assembler="query" method="post">
	...
	<asset:load name="siteplanAsset" type="SiteNavigation" field="id" value='<%=ics.GetVar("spid")%>'  />
	...
	<table class="width-outer-50" bgcolor="#FFFFFF" border="0" cellpadding="0" cellspacing="0">
		...
		<ics:callelement element="OpenMarket/Xcelerate/UIFramework/Util/RowSpacer"/>
		<tr>
			<ics:callelement element="OpenMarket/Xcelerate/AssetType/SiteNavigation/Common/DGList">
		...
		<tr>
			<td class="form-label-text" align="right" NOWRAP="true">
				<xlat:stream key="fatwire/Admin/SiteNavigationIcon" />:
	        </td>
			<td>
				<% 	
					ics.SetVar("AssetType", "SiteNavigation"); 
					ics.SetVar("assetmaker/property/urliconimage/inputform/linktext", "fatwire/admin/Browse");
				%>
				<asset:get name="siteplanAsset" field="urliconimage_file" output="SiteNavigation:urliconimage_file" />
				<asset:get name="siteplanAsset" field="urliconimage" output="SiteNavigation:urliconimage" />
				<asset:get name="siteplanAsset" field="id" output="id" />
				<ics:if condition='<%= Utilities.goodString(ics.GetVar("SiteNavigation:urliconimage_file")) %>' >
				<ics:then>
					<table>
						<tr>
							<td>
					           <ics:callelement element="OpenMarket/AssetMaker/ContentDetailsUpload" >
									<ics:argument name="fieldname" value='urliconimage' />
									<ics:argument name="fieldvalue" value='<%=ics.GetVar("SiteNavigation:urliconimage") %>' />
								</ics:callelement>
							</td>
						</tr>
					</table>
				...
				<satellite:link assembler="query" pagename="OpenMarket/Xcelerate/Admin/SiteplanAdminForm" outstring="goToMainPage">
		   		...
		   		<A HREF='<%=ics.GetVar("goToMainPage")%>' >
					<ics:callelement element="OpenMarket/Xcelerate/UIFramework/Util/TextButton">
						...
						<ics:argument name="dojoButton" value="false"/>
					</ics:callelement>
				</A>
