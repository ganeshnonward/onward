...
<ics:then>
	<asset:list type="SiteNavigation" excludevoided="true" list="siteplanlist" pubid='<%=ics.GetVar("pubid") %>' />
	...
	<ics:then>
		<ics:listloop listname="siteplanlist">
			...
			<siteplan:place name="pubNode" childid='<%=ics.GetVar("sid") %>' type="SiteNavigation" rank='<%=ics.GetVar("nrank") %>' />
		...
		<ics:callelement element='<%="OpenMarket/Xcelerate/UIFramework/UpdateTree" + ics.GetVar("proptreetype") %>' >
			<ics:argument name="__TreeRefreshKeys__" value="Self:MobilitySitePlan;Self:SitePlanSiteName"/>
		</ics:callelement>
	</ics:then>
	</ics:if>
</ics:then>
