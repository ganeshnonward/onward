...
<%@page import="com.openmarket.xcelerate.treetab.TreeTabManager"%>
<%@ page import="COM.FutureTense.Interfaces.ICS" %>
<cs:ftcs>
<%!
    private static final List<String> getUserAccessibleTabs(ICS ics) throws Exception
    {
        IList tabs = null;
        String _enableForms = ics.GetProperty("advancedUI.enableAssetForms","futuretense_xcel.ini", true);
        if("true".equalsIgnoreCase(_enableForms))
            tabs = new TreeTabManager(ics).getOrderedTabs(Long.valueOf(ics.GetSSVar("pubid")));
        else
            tabs = new TreeTabManager(ics).getAllFilteredTabs("ordinal", Long.valueOf(ics.GetSSVar("pubid")));

        //populate accessible tabs for the current user
		boolean isAnalyticsEnabled = new Boolean(ics.GetProperty("insights.enabled", "futuretense_xcel.ini", true));
        List<String> userTabs = new ArrayList<String>();
        for (int i = 0; i <= tabs.numRows(); i++) {
            tabs.moveTo(i);
			String title = tabs.getValue("title");
			if(title.equalsIgnoreCase("Insights") && !isAnalyticsEnabled) {
				//do not add tab to user tabs.
                continue;
			}
            userTabs.add(tabs.getValue("fwuid"));
        }

        //add default tabs accessible by any user
        userTabs.add("CSSystem:TreeTabs:History");
...
<%
    try{
        ...
        //set accessible tabs for the current user
        request.setAttribute("userTabs", getUserAccessibleTabs(ics));
    } catch(Exception e) {
